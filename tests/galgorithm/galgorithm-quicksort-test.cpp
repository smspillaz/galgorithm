/*
 * /tests/galgorithm/galgorithm-quick-test.cpp
 *
 * Tests for the GAlgorithm quicksort function
 *
...
namespace {
  ...
  TEST (GAlgorithmQuicksort, sort_empty_array) {
    g_autoptr(GPtrArray) array = g_ptr_array_new ();

    EXPECT_THAT (PtrArrayWrapper (g_algorithm_quicksort (array,
                                                         ptr_compare)),
                 IsEmpty());
  }

  TEST (GAlgorithmQuicksort, sort_single_array) {
    g_autoptr(GPtrArray) array = g_ptr_array_new ();
  ...
  TEST (GAlgorithmQuicksort, sort_two_elements) {
    g_autoptr(GPtrArray) array = g_ptr_array_new ();
  ...
  TEST (GAlgorithmQuicksort, sort_five_elements) {
    g_autoptr(GPtrArray) array = g_ptr_array_new ();
